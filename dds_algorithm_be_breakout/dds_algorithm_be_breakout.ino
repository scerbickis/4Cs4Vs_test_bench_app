...
uint16_t amplitude1 = 1; // Amplitude of the sine wave
uint16_t amplitude2 = 1; // Amplitude of the sine wave
uint16_t amplitude3 = 1; // Amplitude of the sine wave

uint16_t frequency1 = 50; // Frequency of the sine wave
uint16_t frequency2 = 50; // Frequency of the sine wave
uint16_t frequency3 = 50; // Frequency of the sine wave
...
uint16_t calculateSineTable(){
  ...
  for (uint8_t h = 1; h <= harmonic_order; orderFunction(h)){
      
      for (uint16_t j = 0; j < L; j++) {
        // Scale sine values to the range of 0-65535
        u[j] += (uint16_t)(amplitude/h * sin(h * j * 2 * PI / L));
      }
    }
}
...
void setupParameters() {
  ...
  switch(parameter_id){
    ...
    case 0x50: // 0x50 is Ascii for 'P'.
      phase_id = Serial.read(); // Read the phase id from the serial port
      if (phase_id == 0x31) phase1 = (Serial.read() << 8) | Serial.read(); // Read the load value from the serial port
      else if (phase_id == 0x32) phase2 = (Serial.read() << 8) | Serial.read(); // Read the load value from the serial port
      else if (phase_id == 0x33) phase3 = (Serial.read() << 8) | Serial.read(); // Read the load value from the serial port
      break;
...
void setup() {
  ...
  valdytiSAK(RASYTI_DIAPAZONA_I_VISUS, 0, DIAPAZONAS_10_10); //Visų SAK išėjimų diapazonų nustatymo komanda
...
void loop() {
  ...
  valdytiSAK(RASYTI_DIAPAZONA_I_VISUS, 0, DIAPAZONAS_10_10); // Set the range for all SAK outputs
